...
const calculateResult = async () => {
    ...
    try {
        ...
        const apiUrl = "http://localhost:8080/calculator";
        const response = await fetch(apiUrl, {
            ...
            body: JSON.stringify({
                num1: parseFloat(secondNumber),
                num2: parseFloat(firstNumber),
                operation: operation,
            }),        });
        ...
        if (isNaN(parseFloat(resultText))) {
            Alert.alert("Erreur", resultText);
        } else {
            setResult(parseFloat(resultText));
        }
